...
| `initialBuildingShape` | `BuildingShape` | Formato atual (`"default"`, `"twisted"` ou `"octagonal"`) |
...
| **Formato** | Botões | Opções: padrão (caixa), torre torcida (estilo Cayan Tower, 90° de twist) ou torre octogonal |
...
> - **Formato** → `Mesh` próprio via [[scene-builders#createTwistedBuildingMesh.ts|createTwistedBuildingMesh]] ou [[scene-builders#createOctagonalBuildingMesh.ts|createOctagonalBuildingMesh]] (pula alocação no `InstancedMesh`)
...
> Letreiros e LEDs possuem tratamento específico para `twisted` e `octagonal`, mas holofotes/heliponto ainda usam a **caixa lógica** (`width/depth/height` da bounding box). Em formatos com topo não retangular, acessórios de topo podem ocupar a área da bounding box, não exatamente a silhueta da cobertura.
